...
- 此方案纯CSS做的兼容，不需要改变html结构，也不需要js做处理
- 缺陷：在行高不确定的情况下无法使用此方法
- 缺陷：字数刚好足 `calc(最多行 - 一行)` 时（再多一个字就换行了），会多显示一行空白行
...
到这里，基本差不多了，但有一个巨大缺陷：在刚刚够三行时但还没溢出时，省略号不该显示，但我们的会显示

  ![]({{ site.url }}/assets/img/IE浏览器多行文字溢出显示省略号3.png)
...
- ***为什么是 `:before` 定位， `:after` 浮动呢？俩颠倒用行不？***
  - **答：不行，浮动，是在文档流当前位置浮动起来，浮动起来后脱离文档流，但改变不了它在哪儿脱离的文档流，`:before` 是在dom内最前方插入的，他在最前方浮动起来，即使向右浮动，也是在第一行最右边，而 `:after` 本身是在dom内最末尾插入的，它浮动起来，就会最末行浮动向右，当刚好*三*行时就可以挡住定位在最右边的省略号，超过*三*行，就在溢出的行最右边，省略号就不会被阻挡而显示，所以不能颠倒使用**

- 还有个小小的缺陷：字数刚好足两行（最多显示三行）时（再多一个字就换行了），有时会显示第三行空白行
  - 产生原因：`:after` 有11像素的宽度导致换行

  ![]({{ site.url }}/assets/img/IE浏览器多行文字溢出显示省略号4.png)
